...
class TDatePicker extends CInputWidget {
	...
	private function __registerAsset() {
		$this->__cssFile = Yii::app()->getAssetManager()->publish(
			Yii::getPathOfAlias('bootstrap.assets')/*dirname(__FILE__)*/ . '/css/datetimepicker.css'
		);
		$this->__jsFile = Yii::app()->getAssetManager()->publish(
			Yii::getPathOfAlias('bootstrap.assets') . '/js/bootstrap-datetimepicker.js'
		);
		$this->__localeFile = Yii::app()->getAssetManager()->publish(
			Yii::getPathOfAlias('bootstrap.assets') . '/js/locales/bootstrap-datetimepicker.' . Yii::app()->language . '.js'
		);
	}
